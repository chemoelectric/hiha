...
static string_t_omap_t
delete_from_omap (string_t_omap_t omap, bool (*pred) (int))
{
  string_t_omap_t result = omap;
  for (int i = 1; i != HOW_MANY + 1; i += 1)
    if (pred (i))
...
static void
check_missing (string_t_omap_t omap, bool (*pred) (int))
{
  for (int i = 1; i != HOW_MANY + 1; i += 1)
    if (pred (i))
      {
...
main (void)
{
  ...
  string_t_omap_t omap_even = delete_from_omap (omap, &int_is_odd);
  assert (2 * string_t_omap_size (omap_even) == HOW_MANY);
  assert (string_t_omap_size (omap) == HOW_MANY);
  check_omap (omap_even, &int_is_even, &int_identity);
  check_missing (omap_even, &int_is_odd);

  string_t_omap_t omap_odd = delete_from_omap (omap, &int_is_even);
  assert (2 * string_t_omap_size (omap_odd) == HOW_MANY);
  assert (string_t_omap_size (omap) == HOW_MANY);
  check_omap (omap_odd, &int_is_odd, &int_identity);
  check_missing (omap_odd, &int_is_even);

  string_t_omap_t omap_empty =
    delete_from_omap (omap, &int_is_anything);
  assert (string_t_omap_size (omap_empty) == 0);
  assert (omap_empty == NULL);

  string_t_omap_t omap2 = insert_into_omap (omap_odd, &int_negate);
  assert (string_t_omap_size (omap2) == HOW_MANY);
  check_omap (omap2, &int_is_anything, &int_negate_if_even);

  string_t_omap_t omap3 =
    insert_into_or_replace_in_omap (omap_odd, &int_negate);
  assert (string_t_omap_size (omap3) == HOW_MANY);
  check_omap (omap3, &int_is_anything, &int_negate);

  string_t_omap_t omap4 = replace_in_omap (omap_odd, &int_negate);
  assert (2 * string_t_omap_size (omap4) == HOW_MANY);
  check_omap (omap4, &int_is_odd, &int_negate);
  check_missing (omap4, &int_is_even);

  string_t_vector_t vec4k = string_t_omap_keys (omap4, -1);
  //for (size_t i = 0; i != string_t_vector_length (vec4k); i += 1)
  //  printf ("%s\n", make_str_nul (string_t_vector_ref (vec4k, i)));
  ...
  string_t_keyval_vector_t vec4kv =
    string_t_omap_associations (omap4, -1);
