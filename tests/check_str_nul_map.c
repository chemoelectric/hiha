...
insert_into_map (str_nul_map_t map, int (*f) (int))
{
  str_nul_map_t result = map;
  for (int i = 1; i != HOW_MANY + 1; i += 1)
    {
      ...
      *value = f (i);
      result = str_nul_map_insert_only (result, key, value);
    }
  return result;
}

static str_nul_map_t
replace_in_map (str_nul_map_t map, int (*f) (int))
{
  str_nul_map_t result = map;
  for (int i = 1; i != HOW_MANY + 1; i += 1)
    {
      ...
      int *value = XMALLOC (int);
      *value = f (i);
      result = str_nul_map_replace_only (result, key, value);
    }
  return result;
}

static str_nul_map_t
insert_into_or_replace_in_map (str_nul_map_t map, int (*f) (int))
{
  str_nul_map_t result = map;
  for (int i = 1; i != HOW_MANY + 1; i += 1)
    {
      ...
      int *value = XMALLOC (int);
      *value = f (i);
      result = str_nul_map_insert_or_replace (result, key, value);
    }
  return result;
}

static str_nul_map_t
delete_from_map (str_nul_map_t map, bool (*pred) (int))
{
  str_nul_map_t result = map;
  for (int i = 1; i != HOW_MANY + 1; i += 1)
    if (pred (i))
      {
        ...
        snprintf (buf, 100, "key%06d", i);
        const char *key = xstrdup (buf);
        result = str_nul_map_delete (result, key);
      }
  return result;
}

static void
check_map (str_nul_map_t map, bool (*pred) (int), int (*f) (int))
{
  for (int i = 1; i != HOW_MANY + 1; i += 1)
    if (pred (i))
      {
        ...
        snprintf (buf, 100, "key%06d", i);
        const char *key = xstrdup (buf);
        const void *value = str_nul_map_search (map, key);
        assert (value != NULL);
...
static void
check_missing (str_nul_map_t map, bool (*pred) (int))
{
  for (int i = 1; i != HOW_MANY + 1; i += 1)
    if (pred (i))
      {
...
main (void)
{
  ...
  str_nul_map_t map = NULL;
  assert (str_nul_map_size (map) == 0);

  map = insert_into_map (map, &int_identity);
  assert (str_nul_map_size (map) == HOW_MANY);
  ...
  str_nul_map_t map_even = delete_from_map (map, &int_is_odd);
  assert (2 * str_nul_map_size (map_even) == HOW_MANY);
  assert (str_nul_map_size (map) == HOW_MANY);
  check_map (map_even, &int_is_even, &int_identity);
  check_missing (map_even, &int_is_odd);

  str_nul_map_t map_odd = delete_from_map (map, &int_is_even);
  assert (2 * str_nul_map_size (map_odd) == HOW_MANY);
  assert (str_nul_map_size (map) == HOW_MANY);
  check_map (map_odd, &int_is_odd, &int_identity);
  check_missing (map_odd, &int_is_even);

  str_nul_map_t map_empty = delete_from_map (map, &int_is_anything);
  assert (str_nul_map_size (map_empty) == 0);
  assert (map_empty == NULL);

  str_nul_map_t map2 = insert_into_map (map_odd, &int_negate);
  assert (str_nul_map_size (map2) == HOW_MANY);
  check_map (map2, &int_is_anything, &int_negate_if_even);

  str_nul_map_t map3 =
    insert_into_or_replace_in_map (map_odd, &int_negate);
  assert (str_nul_map_size (map3) == HOW_MANY);
  check_map (map3, &int_is_anything, &int_negate);

  str_nul_map_t map4 = replace_in_map (map_odd, &int_negate);
  assert (2 * str_nul_map_size (map4) == HOW_MANY);
  check_map (map4, &int_is_odd, &int_negate);
  check_missing (map4, &int_is_even);

  str_nul_vector_t vec4k = str_nul_map_keys (map4);
  str_nul_map_t map4a = map4;
  for (size_t i = 0; i != str_nul_vector_length (vec4k); i += 1)
    map4a = str_nul_map_delete (map4a, str_nul_vector_ref (vec4k, i));
  assert (map4a == NULL);

  voidp_vector_t vec4v = str_nul_map_values (map4);
  for (size_t i = 0; i != str_nul_vector_length (vec4k); i += 1)
    {
      const char *key = str_nul_vector_ref (vec4k, i);
      const void *value = str_nul_map_search (map4, key);
  ...
  for (size_t i = 0; i != str_nul_keyval_vector_length (vec4kv); i += 1)
    {
      ...
      int val2 = *((const int *) str_nul_map_search (map4, key));
      assert (val1 == val2);
    }
